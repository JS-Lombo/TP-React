...
function App() {
  const initialTaskList = JSON.parse(localStorage.getItem('taskslist') || '[]')
  const initialPrimaryTheme = JSON.parse(localStorage.getItem('primaryTheme') || 'true')
  ...
  const [primaryTheme, setPrimaryTheme] = useState(initialPrimaryTheme)
  const [positionEdit, setPositionEdit] = useState()

  const handleAddTask = (taskIndex) =>{
    setEditTask(false)
    if (task.trim() === "") return
    if (taskIndex > -1) {
